...
def lstm_cell_forward(xt, a_prev, c_prev, parameters):
    ...
    ft = sigmoid(np.dot(Wf,concat)+bf) # Γ⟨t⟩f=σ(Wf[a⟨t−1⟩,x⟨t⟩]+bf)
    it = sigmoid(np.dot(Wi,concat)+bi) # Γ⟨t⟩u=σ(Wu[a⟨t−1⟩,x{t}]+bu)
    cct = np.tanh(np.dot(Wc,concat)+bc) # c̃ ⟨t⟩=tanh(Wc[a⟨t−1⟩,x⟨t⟩]+bc)
    c_next = c_next = np.multiply(ft,c_prev)+ np.multiply(it,cct) # c⟨t⟩=Γ⟨t⟩f∗c⟨t−1⟩+Γ⟨t⟩u∗c̃ ⟨t⟩
    ot = ot = sigmoid(np.dot(Wo,concat)+bo) # Γ⟨t⟩o=σ(Wo[a⟨t−1⟩,x⟨t⟩]+bo)
    a_next = a_next = ot*np.tanh(c_next) # a⟨t⟩=Γ⟨t⟩o∗tanh(c⟨t⟩)
    
    # Compute prediction of the LSTM cell (≈1 line)
    yt_pred = yt_pred = softmax(np.dot(Wy, a_next) + by)
    ### END CODE HERE ###
